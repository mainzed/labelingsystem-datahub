...
public class LSDump implements Runnable {
	...
	private void writeFile() {
		try {
			...
			Path destinationpath = Paths.get(fileName.replace(String.valueOf(currentTime), "labelingsystem-datahub-latest").replace(DumpResource.tmpDirString, DumpResource.tmpDirString2));
			Files.copy(sourcePath, destinationpath);
			// delete file
			File fileDelete = new File(fileName.replace(String.valueOf(currentTime), "labelingsystem-datahub-latest"));
			Files.deleteIfExists(fileDelete.toPath());
			// write readme.txt
			PrintWriter writer = new PrintWriter(DumpResource.tmpDirPath + "readme.txt", "UTF-8");
			writer.println("Labeling System Data Hub Readme");
			writer.close();
			PrintWriter writer2 = new PrintWriter(DumpResource.tmpDirPath2 + "readme.txt", "UTF-8");
			writer2.println("Labeling System Data Hub Readme");
			writer2.close();
			// compress folder(s)
			DumpResource.dumbNo++;
			String compressedFile = DumpResource.fileDir + "LSDH-" + String.valueOf(currentTime) + "-" + DumpResource.getNumberOfStatements() + "-" + DumpResource.dumbNo + ".tar.gz"; // destination tar.gz
			TarGZ.compressWithoutFolder(DumpResource.tmpDirPath, compressedFile, DumpResource.tmpDirString);
			String compressedFile2 = DumpResource.fileDir + "labelingsystem-datahub-latest.tar.gz"; // destination tar.gz
			...
			// set public path
			DumpResource.lastDump = DumpResource.filePath + "LSDH-" + String.valueOf(currentTime) + "-" + DumpResource.getNumberOfStatements() + "-" + DumpResource.dumbNo + ".tar.gz"; // destination tar.gz
